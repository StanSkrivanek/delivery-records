...
export class RecordService {
  ...
  static async updateRecord(id: number, fields: Partial<RecordRow & { odometer?: number }>): Promise<any> {
  ...
  static async createVehicleUsageLog(input: {
    ...
    await pgPool.query(
      ...
      [record_id, vehicle_id, user_id, entry_date, usage_mode, odoStart, odometer_end ?? null, distance_manual, purpose ?? null, comment ?? null]
    );
  }
  ...
  static async getOdometerDifferencesByMonth(year: number, month: number): Promise<{ date: string; distance: number }[]> {
    const { rows } = await pgPool.query(
  ...
  static async getOdometerStatsByMonth(year: number, month: number): Promise<{
