...
export function createBetterAuth(pgPool: Pool, config: BetterAuthConfig = {}) {
	...
	return {
		...
		async getUserByEmail(email: string) {
			const { rows } = await pgPool.query('select * from public.users where email = $1 limit 1', [
				email
			]);
			return rows[0] ?? null;
		},
		...
		async revokeSession(userId: number, sessionId: string) {
			await pgPool.query(`delete from public.sessions where id = $1 and user_id = $2`, [
				sessionId,
				userId
			]);
		},
...
export async function resetPasswordWithToken(
	...
	try {
		...
		await pgPool.query('update public.password_reset_tokens set used_at = now() where id = $1', [
			rec.token_id
		]);
